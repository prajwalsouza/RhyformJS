import { clamp, duration } from "./numeric.js";

// The schedule and clock are shared by SVG and GPU scenes. Rendering is an adapter.
export function installTimeline(scene, adapter = {}) {
  let frame = null,
    origin = 0,
    tracks = [],
    compiledCount = -1,
  ...
  function compile() {
    ...
    for (const animation of animations) {
      ...
      cursor = (
        animation.group
          ? animation.group.startGroupAnimationsImmediately
          : animation.animateNextImmediately
      )
        ? start
        : end;
    }
    ...
    for (const [element, o] of state) {
      ...
      if (o.action === "play") {
        ...
        try {
          ...
          /* media metadata may not have arrived yet */
        }
        if (play)
