...
viewX.libraryFunctions.interpolateArrayOfPoints = function(xv1, xv2, A, B, inputv) {
    ...
    for (let i = 0; i < maxLength; i++) {
        let indexA = maxLength === 1 ? 0 : i * (A.length - 1) / (maxLength - 1);
        let indexB = maxLength === 1 ? 0 : i * (B.length - 1) / (maxLength - 1);
...
viewX.updateGraphZoom = function(graphname, newMinMax) {
	...
	for (var key in gdata.pathData) {
        // Keep command paths (including separate M contours) intact on resize.
